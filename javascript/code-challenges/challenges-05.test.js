...
const toLastNames = people => {
  // Solution code here...
  let newArray = people.map(i => `${i.firstName} ${i.lastName}`);
  return newArray;
};
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let charArray = arr.filter(i => i.children);
  let childArray = charArray.map(i => i.children);
  let fullCArray = [].concat(...childArray);
  return fullCArray.reduce((a, b, c) => {
    a + b;
    return c + 1;
  }, 0);
};
